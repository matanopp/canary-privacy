...
class ToDoList extends React.Component {
    ...
    render() {
        return (
            <div className="to-do-list">
                <h1>To Do List</h1>
                {this.props.actionItems.sort(this.compareItems).map(item => this.makeToDoItem(item))}
            </div>
        );
    }
    ...
    compareItems(a, b) {
        ...
        if (a.priority.toLowerCase() === 'high' || b.priority.toLowerCase() === 'low') return -1;
        if (a.priority.toLowerCase() === 'low' || b.priority.toLowerCase() === 'high') return 1;
        return 0;
    }
